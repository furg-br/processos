...
export function NewProcessDialog({ isOpen, onClose, onCreated }: {
  ...
  >
    <form className="new-process-form" onSubmit={submit}>
      ...
      <FurgSelect disabled={loadingOrganizations || organizations.length === 0} helperText={loadingOrganizations ? "Carregando unidades..." : "A escolha é obrigatória e não é presumida pela ordem do cadastro."} label="Unidade responsável" onChange={(event) => update("ownerUnitId", event.target.value)} options={[{ value: "", label: "Selecione uma unidade" }, ...organizations.map((unit) => ({ value: unit.id, label: `${unit.acronym} - ${unit.name}` }))]} required value={form.ownerUnitId} />
      <FurgSelect label="Cenário inicial" onChange={(event) => update("perspective", event.target.value as Perspective)} options={[{ value: "AS_IS", label: "Processo atual" }, { value: "TO_BE", label: "Cenário futuro" }]} value={form.perspective} />
      ...
      <div className="new-process-form__actions new-process-form__wide"><FurgActionRow align="end"><FurgButton disabled={submitting} onClick={onClose} type="button" variant="text">Cancelar</FurgButton><FurgButton disabled={loadingOrganizations || organizations.length === 0 || !form.ownerUnitId} icon="document" loading={submitting} type="submit">Criar e abrir diagrama</FurgButton></FurgActionRow></div>
    </form>
  </FurgDialog>;
}
