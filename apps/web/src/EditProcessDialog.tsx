...
export function EditProcessDialog({ isOpen, process, onClose, onUpdated }: {
  ...
  >
    <form className="new-process-form" onSubmit={submit}>
      ...
      <FurgSelect label="Cenário" onChange={(event) => update("perspective", event.target.value as Perspective)} options={[{ value: "AS_IS", label: "AS-IS — processo atual" }, { value: "TO_BE", label: "TO-BE — cenário futuro" }]} value={form.perspective} />
